...
<script>
   ...
   export default {
     ...
     methods: {
       fetchTasks: function () {
         axios.get('api/todos/index').then((response) => {
       ...
       createTask: function () {
         if (!this.newTask) return;
           axios.post('/api/todos/create', { todo: { content: this.newTask } }).then((response) => {
       ...
       doneTask: function (task_id) {
         axios.put('/api/todos/update/' + task_id, { todo: { is_done: 1 } }).then((response) => {
           this.moveFinishedTask(response.data.id);
         }, (error) => {
           console.log(error);
         });
       },
